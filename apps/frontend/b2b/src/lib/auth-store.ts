  ...
  persist(
    (set, get) => ({
      ...
      signup: async (data) => {
        ...
        try {
          const response = await apiService.instance.post('/auth/register', data)
        ...
        return {
          ...
          setItem: (name, value) => {
            ...
            if (typeof window !== 'undefined') {
              ...
              document.cookie = `travel-auth=${encodeURIComponent(JSON.stringify(parsed.state))};path=/;max-age=${7 * 24 * 60 * 60};SameSite=Lax`
            }
          },
          removeItem: (name) => {
            if (typeof window !== 'undefined') {
              ...
              // Clear cookie
              document.cookie = `travel-auth=;expires=Thu, 01 Jan 1970 00:00:01 GMT;path=/;SameSite=Lax`
            }
          }
        }
      }),
