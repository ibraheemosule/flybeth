...
export function MyTripsPage() {
  ...
  const totalPages = Math.ceil(bookingHistory.length / ITEMS_PER_PAGE);
        ...
        <Tabs defaultValue="upcoming" className="w-full">
              ...
              upcomingTrips.map((trip) => (
                <Card key={trip.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                  <div className="flex flex-col md:flex-row">
                    <div className="bg-gradient-to-br from-primary to-accent p-8 md:w-64 flex flex-col justify-between text-white">
                      <div>
                      ...
                      <div className="space-y-4 mb-6">
                        {trip.details.flight && (
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <div>Flight</div>
                              <p className="text-muted-foreground text-sm">{trip.details.flight}</p>
                            </div>
                          </div>
                        )}
                        {trip.details.hotel && (
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <div>Hotel</div>
                              <p className="text-muted-foreground text-sm">{trip.details.hotel}</p>
                            </div>
                          </div>
                        )}
                      </div>
          ...
          <TabsContent value="history" className="space-y-6">
            {currentBookings.map((booking) => (
              <Card key={booking.id} className="overflow-hidden opacity-90 hover:opacity-100 transition-opacity hover:shadow-lg">
                <div className="flex flex-col md:flex-row">
                  ...
                  <div className="flex-1 p-6">
                    ...
                    <div className="space-y-3 mb-6">
                      {booking.category === 'flight' && booking.details.flight && (
                        <div className="flex items-start gap-3">
                          <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                            <Plane className="h-5 w-5 text-primary" />
                          </div>
                          <div>
                            ...
                            <p className="text-muted-foreground text-sm">{booking.details.flight}</p>
                          </div>
                        </div>
                      )}
                      
                      {booking.category === 'hotel' && booking.details.hotel && (
                        <>
                          <div className="flex items-start gap-3">
                            <div className="w-10 h-10 rounded-lg bg-accent/10 flex items-center justify-center flex-shrink-0">
                              <Hotel className="h-5 w-5 text-accent" />
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground text-sm">{booking.details.hotel}</p>
                            </div>
                          </div>
                      ...
                      {booking.category === 'car' && booking.details.car && (
                        <>
                          <div className="flex items-start gap-3">
                            <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                            ...
                            <div>
                              ...
                              <p className="text-muted-foreground text-sm">{booking.details.car}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 text-sm text-muted-foreground ml-13">
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-8 p-4 bg-white rounded-xl border shadow-sm">
                <Button
                  variant="outline"
