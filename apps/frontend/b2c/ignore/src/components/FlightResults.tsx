...
import { Plane, Clock, Briefcase, Wifi, Coffee, ArrowRight, MapPin, ArrowLeftRight, ChevronDown, ChevronUp } from "lucide-react";
...
const generateFlights = (from: string, to: string, date: string): Flight[] => [
  ...
  },
  {
    ...
    price: 279,
    departure: { time: "05:30 AM", airport: from },
    arrival: { time: "03:45 PM", airport: to },
    duration: "10h 15m",
    stops: [
      { airport: "ATL", city: "Atlanta", layover: "2h 10m" },
      { airport: "DEN", city: "Denver", layover: "1h 45m" }
    ],
    class: "Economy",
    amenities: ["entertainment"],
    baggage: "1 checked bag included",
  ...
  {
    id: 6,
    airline: "Coast to Coast",
    price: 349,
    departure: { time: "11:15 AM", airport: from },
    arrival: { time: "06:20 PM", airport: to },
    duration: "7h 05m",
    stops: [
      { airport: "DFW", city: "Dallas", layover: "1h 30m" }
    ],
    class: "Economy",
    amenities: ["wifi", "entertainment"],
    baggage: "1 checked bag included",
...
const generateRoundTripFlights = (from: string, to: string, departDate: string, returnDate: string): RoundTripFlight[] => {
  ...
  // Create some popular combinations
...
const generateMultiCityFlights = (): MultiCityFlight[] => {
  // Sample multi-city routes: NYC -> LA -> Tokyo -> NYC
  const multiCityRoutes = [
    ...
    {
      legs: [
        generateFlights("JFK", "MIA", "Dec 20, 2025")[2],
        generateFlights("MIA", "LAX", "Dec 23, 2025")[4],
        generateFlights("LAX", "SFO", "Dec 27, 2025")[1],
        generateFlights("SFO", "JFK", "Dec 30, 2025")[0],
      ]
    },
    {
      legs: [
        generateFlights("JFK", "ATL", "Dec 20, 2025")[5],
        generateFlights("ATL", "DEN", "Dec 25, 2025")[3],
        generateFlights("DEN", "JFK", "Dec 29, 2025")[1],
      ]
    },
    // Add a long multi-city route with 6 legs to show collapsed functionality
    {
      legs: [
        generateFlights("JFK", "LAX", "Dec 18, 2025")[0],
        generateFlights("LAX", "SFO", "Dec 20, 2025")[1],
        generateFlights("SFO", "SEA", "Dec 22, 2025")[2],
        generateFlights("SEA", "DEN", "Dec 24, 2025")[3],
        generateFlights("DEN", "MIA", "Dec 27, 2025")[4],
        generateFlights("MIA", "JFK", "Dec 30, 2025")[5],
      ]
    },
    // Add another long route with 7 legs
    {
      legs: [
        generateFlights("JFK", "BOS", "Jan 5, 2026")[1],
        generateFlights("BOS", "ORD", "Jan 7, 2026")[2],
        generateFlights("ORD", "DEN", "Jan 9, 2026")[0],
        generateFlights("DEN", "LAX", "Jan 11, 2026")[3],
        generateFlights("LAX", "SFO", "Jan 13, 2026")[1],
        generateFlights("SFO", "SEA", "Jan 15, 2026")[2],
        generateFlights("SEA", "JFK", "Jan 18, 2026")[4],
      ]
    },
  ];
...
const FlightLegDisplay = ({ flight, label, from, to }: { flight: Flight; label: string; from: string; to: string }) => (
                ...
                >
                  <div className="relative group">
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                      ...
                      <div className="text-[10px] text-gray-300">{stop.layover} layover</div>
                    </div>
                  </div>
                </div>
              ))}
...
// Multi-city card with collapsible legs
const MultiCityFlightCard = ({ multiCity, index, onSelectFlight }: { 
  multiCity: MultiCityFlight; 
  index: number; 
  onSelectFlight: (flight: any) => void;
}) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const maxLegsToShow = 3; // Show first 3 legs by default
  const hasMoreLegs = multiCity.legs.length > maxLegsToShow;
  
  // Calculate total duration
  const totalStops = multiCity.legs.reduce((sum, leg) => sum + leg.stops.length, 0);
            ...
            <div className="flex-1 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <Badge variant="outline" className="bg-gradient-to-r from-accent/10 to-primary/10 border-accent/20">
                ...
                <div className="flex items-center gap-1 text-sm overflow-x-auto">
                  {multiCity.legs.map((leg, idx) => (
                    <span key={idx} className="flex items-center gap-1 shrink-0">
                      <span className="font-medium">{leg.departure.airport}</span>
                      ...
                      {idx === multiCity.legs.length - 1 && (
                        <>
                          <ArrowRight className="h-3 w-3 text-muted-foreground" />
                          <span className="font-medium">{leg.arrival.airport}</span>
                        </>
                      )}
                    </span>
                  ))}
              ...
              {/* Flight Legs - Show first few or all if expanded */}
              <div className="space-y-4">
                {(isExpanded ? multiCity.legs : multiCity.legs.slice(0, maxLegsToShow)).map((leg, legIndex) => (
                  <div key={legIndex}>
                    <FlightLegDisplay 
                      flight={leg} 
                    ...
                    />
                    {legIndex < (isExpanded ? multiCity.legs.length - 1 : Math.min(maxLegsToShow, multiCity.legs.length) - 1) && (
                    ...
                    >
                      {isExpanded ? (
                        ...
                        <>
                          ...
                          Show {multiCity.legs.length - maxLegsToShow} More {multiCity.legs.length - maxLegsToShow === 1 ? 'Leg' : 'Legs'}
                        </>
                      )}
                    </Button>
                  </div>
...
export function FlightResults({ searchParams, onClose, onSelectFlight }: FlightResultsProps) {
  ...
  const oneWayFlights = !isRoundTrip && !isMultiCity ? generateFlights(searchParams.from, searchParams.to, searchParams.departDate) : [];
  const roundTripFlights = isRoundTrip && !isMultiCity
    ? generateRoundTripFlights(searchParams.from, searchParams.to, searchParams.departDate, searchParams.returnDate!)
    : [];
      ...
      <div className="container mx-auto max-w-5xl">
        ...
        >
              ...
              <p className="text-muted-foreground">
                {isMultiCity ? (
                  ...
                  <>
                    ...
                    {isRoundTrip && ` → ${searchParams.from}`} • {searchParams.departDate}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Badge variant="secondary" className="bg-gradient-to-r from-primary/10 to-accent/10 border-primary/20">
              {isMultiCity ? multiCityFlights.length : isRoundTrip ? roundTripFlights.length : oneWayFlights.length} flights found
            </Badge>
            <Badge variant="secondary" className="bg-gradient-to-r from-primary/10 to-accent/10 border-primary/20">
              Best prices
            </Badge>
        ...
        {isMultiCity && (
          <div className="space-y-4">
            {multiCityFlights.map((multiCity, index) => (
              <MultiCityFlightCard key={multiCity.id} multiCity={multiCity} index={index} onSelectFlight={onSelectFlight} />
            ))}
          </div>
        )}
                    ...
                    <div className="flex flex-col lg:flex-row gap-6">
                      ...
                      <div className="flex-1 space-y-6">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <Badge 
                              variant="outline" 
                              className={roundTrip.outbound.class === "Business" 
                            ...
                            {roundTrip.outbound.stops.length === 0 && roundTrip.return.stops.length === 0 && (
                        ...
                        <div className="flex flex-wrap gap-3 text-sm text-muted-foreground pt-2 border-t border-gray-100">
                          ...
                          {roundTrip.outbound.amenities.includes("entertainment") && (
                            <div className="flex items-center gap-1">
                              <span>📺 Entertainment</span>
                            </div>
                          )}
                      ...
                      <div className="lg:w-48 flex lg:flex-col items-center lg:items-end justify-between lg:justify-start gap-4 p-4 lg:p-0 rounded-xl bg-gradient-to-br from-primary/5 to-accent/5 lg:bg-none">
                        <div className="text-center lg:text-right">
                          ...
                          <p className="text-sm text-muted-foreground">per person</p>
                          <p className="text-xs text-muted-foreground mt-1">
                            ${roundTrip.outbound.price} + ${roundTrip.return.price}
                          </p>
                        </div>
                        <Button 
                          className="bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 shadow-lg hover:shadow-xl transition-all"
