...
import { http, HttpResponse } from "@packages/shared-frontend";
import { server } from "@packages/shared-frontend/test/server";
...
describe("TravelSearchResults Component", () => {
  ...
  it("loads and displays initial travel packages", async () => {
    ...
    expect(screen.getByTestId("rating-1")).toHaveTextContent(
      "4.8 stars (324 reviews)"
    );
    expect(screen.getByTestId("destination-1")).toHaveTextContent(
      "Bali, Indonesia"
    );
  });
  ...
  it("filters packages by destination", async () => {
    ...
    expect(screen.getByText("European Adventure")).toBeInTheDocument();
    expect(
      screen.queryByText("Tropical Paradise in Bali")
  ...
  it("shows loading state during search", async () => {
    ...
    expect(
      screen.getByText("Searching for amazing trips...")
    ).toBeInTheDocument();
  ...
  it("handles search API errors", async () => {
    ...
    expect(
      screen.getByText(/Failed to search travel packages/)
    ).toBeInTheDocument();
  });
