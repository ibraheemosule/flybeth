...
const LoadingSpinner = ({ message = 'Checking authentication...' }: LoadingSpinnerProps) => (
...
const AuthGuard = ({
  ...
  const { isAuthenticated, user, isLoading, checkTokenValidity } = useAuthStore()
  ...
  const { checkAndRefresh, isTokenValid } = useTokenRefresh({
    ...
    onRefreshError: () => {
      ...
      const loginUrl = `${redirectTo}${currentPath !== redirectTo ? `?redirect=${encodeURIComponent(currentPath)}` : ''}`
