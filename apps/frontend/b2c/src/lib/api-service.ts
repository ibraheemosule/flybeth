...
class ApiService extends BaseApiService {
  ...
  private getAuthState(): AuthState | null {
    try {
      ...
      if (typeof document !== 'undefined') {
        const cookieAuth = this.getCookie('travel-auth')
        if (cookieAuth) {
      ...
      if (typeof window !== 'undefined') {
        const localAuth = localStorage.getItem('travel-auth')
        if (localAuth) {
  ...
  private updateAuthState(newAuthState: AuthState) {
    try {
      ...
      if (typeof document !== 'undefined') {
        document.cookie = `travel-auth=${authData}; path=/; max-age=${7 * 24 * 60 * 60}; SameSite=Strict`
      }
      ...
      if (typeof window !== 'undefined') {
        ...
        localStorage.setItem('travel-auth', JSON.stringify(zustandData))
      }
  ...
  private handleLogout() {
    // Clear cookies
    if (typeof document !== 'undefined') {
      document.cookie = 'travel-auth=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT;'
    }
