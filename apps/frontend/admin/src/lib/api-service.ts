...
class ApiService extends BaseApiService {
  ...
  private updateAuthState(newAuthState: AuthState) {
    try {
      ...
      if (typeof document !== 'undefined') {
        document.cookie = `travel-admin-auth=${authData}; path=/; max-age=${7 * 24 * 60 * 60}; SameSite=Strict`
      }
  ...
  private handleLogout() {
    // Clear cookies
    if (typeof document !== 'undefined') {
      document.cookie = 'travel-admin-auth=; path=/; expires=Thu, 01 Jan 1970 00:00:01 GMT;'
    }
  ...
  async login(email: string, password: string, mfaCode?: string): Promise<{ user: User; accessToken: string; refreshToken: string }> {
    try {
      const response = await this.post('/api/auth/login', { email, password, mfaCode })
  ...
  async updateBookingStatus(bookingId: string, status: string) {
    try {
      const response = await this.patch(`/api/admin/bookings/${bookingId}/status`, { status })
      return response.data
    } catch (error) {
      throw createApiError(error)
    }
  ...
  async updateCompany(companyId: string, companyData: any) {
    try {
      const response = await this.patch(`/api/admin/companies/${companyId}`, companyData)
      return response.data
    } catch (error) {
      throw createApiError(error)
    }
  ...
  async suspendCompany(companyId: string) {
    try {
      const response = await this.patch(`/api/admin/companies/${companyId}/suspend`)
      return response.data
    } catch (error) {
      throw createApiError(error)
    }
