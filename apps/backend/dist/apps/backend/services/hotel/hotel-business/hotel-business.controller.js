...
exports.HotelBusinessController = void 0;
const hotel_helper_1 = require("../hotel-shared/hotel.helper");
const hotel_business_schemas_1 = require("./hotel-business.schemas");
class HotelBusinessController {
    constructor() {
        // Search hotels with business features
        this.searchHotels = async (req, res) => {
            try {
                const filters = hotel_business_schemas_1.businessHotelSearchSchema.parse(req.query);
                ...
                const businessHotels = hotels.map(hotel => ({
                    ...
                    businessAmenities: hotel.amenities.filter(amenity => ['Business Center', 'Conference Rooms', 'WiFi', 'Concierge'].includes(amenity)),
        ...
        this.getHotelDetails = async (req, res) => {
            try {
                ...
                if (checkIn && checkOut && guests) {
                    ...
                    const basePricing = await this.hotelHelper.getHotelPricing(hotelId, checkIn, checkOut, guestCount);
                    if (basePricing) {
                        // Apply corporate discounts
                        let discount = 15; // Base corporate discount
                        if (corporateId)
                            discount += 5; // Additional 5% for registered corporate clients
                        if (basePricing.totalNights >= 7)
                            discount += 10; // Long stay discount
                        if (guestCount >= 20)
        ...
        this.createReservation = async (req, res) => {
            try {
                const bookingData = hotel_business_schemas_1.businessHotelBookingSchema.parse(req.body);
                ...
                const basePricing = await this.hotelHelper.getHotelPricing(bookingData.hotelId, bookingData.checkIn, bookingData.checkOut, bookingData.guests);
                if (!basePricing) {
                    return res.status(400).json({
                        success: false,
                ...
                if (bookingData.corporateId)
                    discount += 5;
                if (basePricing.totalNights >= 7)
                    discount += 10;
                if (bookingData.guests >= 20)
                    discount += 5;
                const discountAmount = Math.round(basePricing.totalPrice * (discount / 100));
        ...
        this.createBulkBooking = async (req, res) => {
            try {
                const bulkData = hotel_business_schemas_1.bulkBookingSchema.parse(req.body);
                const businessId = req.business?.id;
                if (!businessId) {
                    return res.status(401).json({
                        success: false,
                ...
                for (const booking of bulkData.bookings) {
                    const basePricing = await this.hotelHelper.getHotelPricing(booking.hotelId, booking.checkIn, booking.checkOut, booking.guests);
                    if (!basePricing)
                        continue;
                    // Enhanced bulk discount
                    let discount = 25; // Base bulk discount
                    if (bulkData.corporateDiscount)
                        discount += bulkData.corporateDiscount;
                    if (bulkData.bookings.length >= 10)
        ...
        this.getCorporateAnalytics = async (req, res) => {
            try {
        ...
        this.requestCustomRates = async (req, res) => {
            try {
                const { estimatedAnnualVolume, preferredLocations, companyInfo, contactDetails } = req.body;
...
exports.HotelBusinessController = HotelBusinessController;
