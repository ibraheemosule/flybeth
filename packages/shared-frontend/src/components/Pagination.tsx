...
export const Pagination: React.FC<PaginationProps> = ({
  ...
  const buttonBaseClass =
    "px-3 py-2 rounded text-sm font-medium transition-colors focus:outline-none focus:ring-2";
  ...
  return (
    <div
      className={`flex items-center justify-between mt-8 p-4 bg-white rounded-xl border shadow-sm ${className}`}
    >
      <button
        onClick={handlePrevPage}
        disabled={currentPage === 1}
        className={`${prevNextClass} flex items-center gap-2`}
      >
        <svg
          className="h-4 w-4"
          fill="none"
