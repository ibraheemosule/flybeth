import { formatError } from '@packages/shared-utils';
...
export const createFrontendApiError = (error) => {
    if (error.response) {
        ...
        return new FrontendApiError(data.message || data.error || 'API request failed', status, data.code, data);
    }
    if (error.request) {
