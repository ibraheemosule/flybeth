 ...
 */
export function getUserFromToken(token: string): { userId: string; userType: string; email?: string } | null {
  try {
    const decoded = decodeJWT(token)
...
export function hasRole(token: string, requiredRoles: string[]): boolean {
  try {
    ...
    return requiredRoles.some(role =>
      ...
      userRoles.map((r: string) => r.toUpperCase()).includes(role.toUpperCase())
    )
  } catch (error) {
    return false
  }
